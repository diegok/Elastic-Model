...
SKIP: {
    skip "facets not supported in 2.x", 1 if $ENV{ES_CLIENT_VERSION} > 1;
    isa_ok $view = $view->facets( name => { terms => { field => 'name' } } ),
...
SKIP: {
    skip "facets not supported in 2.x", 2 if $ENV{ES_CLIENT_VERSION} > 1;
...
## DELETE ##
SKIP: {
    skip "delete_by_query() not supported on 2.x without the plugin", 3 unless $ENV{ES_DELETE_BY_QUERY} || $ENV{ES_CLIENT_VERSION} < 2;
