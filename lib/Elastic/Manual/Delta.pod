...
Elastic::Model is now compatible with Elasticsearch 2.x. which it's the default client
of L<Search::Elasticsearch>. You should specify the client version explicitly if you
use Elasticsearch 1.x the same way you did for version 0.52. On the other hand, support
for Elasticsearch 0.90.x is completely removed so you should not upgrade further than
0.52 if you still use that version.
