...
that we don't use and will never use, some of that I still don't completely
understand. Finally I was able to make almost all tests pass using 2.4 and I'm
still working on it.

I need to test back on 1.x but now I can see this will work. On the other hand
I've discovered that maintaining support for 0.9 will be a harder work that
I'm not interested to do, so my plan is to completely remove support for it and try
to reach a point where I can request Clinton to merge my changes to make a new
CPAN release supporting 2.x possible.
...
For instance:
    ...
    curl -L -O https://github.com/downloads/elasticsearch/elasticsearch/elasticsearch-1.7.4.tar.gz
    tar -xzf elasticsearch-1.7.4.tar.gz
